...
# Set wd 
setwd("/Users/phowlett/Library/CloudStorage/Dropbox/PhD/BSc 2024/Silicosis - Systematic Review/Meta-Analysis")

# Load files from Documents

HRCT_Normal <- read_xlsx(path = "HRCT.xlsx")
CT_Normal <- read_xlsx(path = "CT.xlsx")
...
png(filename = "HRCT_Sensitivity_1.png", width = 3600, height = 2262, res = 400)
...
png(filename = "HRCT_Specificity_1.png", width = 3600, height = 2262, res = 400)
...
png(filename = "CT_Sensitivity_1.png",  width = 3600, height = 2262, res = 400)
...
png(filename = "CT_Specificity_1.png",  width = 3600, height = 2262, res = 400)
...
png(filename = "Autopsy_Sensitivity_1.png",  width = 3600, height = 2262, res = 400)
...
png(filename = "Autopsy_Specificity_1.png",  width = 3600, height = 2262, res = 400)
...
plot_hrsens <- png::readPNG("HRCT_Sensitivity_1.png")
plot_hrspec <- png::readPNG("HRCT_Specificity_1.png")
plot_ctsens <- png::readPNG("CT_Sensitivity_1.png")
plot_ctspec <- png::readPNG("CT_Specificity_1.png")
plot_autsens <- png::readPNG("Autopsy_Sensitivity_1.png")
plot_autspec <- png::readPNG("Autopsy_Specificity_1.png")
...
png(filename = "HRCT_Sensitivity_and_Specificity.png", width = 7200, height = 2262, res = 400)
grid.draw(hrct_plot)
dev.off()
...
# Save the combined plot
png(filename = "CT_Sensitivity_and_Specificity.png", width = 7200, height = 2262, res = 400)
grid.draw(ct_plot)
dev.off()
...
# Save the combined plot
png(filename = "Autopsy_Sensitivity_and_Specificity.png", width = 7200, height = 2262, res = 400)
grid.draw(aut_plot)
dev.off()
...
# Select rows in HRCT_Normal where rob = 0 
HRCT_Normal <- HRCT_Normal[HRCT_Normal$rob == 0,]

# Run analysis again with new df
HRCT_Sensitivity <- metaprop(HRCT_Normal$TP, HRCT_Normal$sens_tot, comb.fixed=FALSE, comb.random = TRUE, sm = "plogit", studlab=HRCT_Normal$ID)
HRCT_Specificity <- metaprop(HRCT_Normal$TN, HRCT_Normal$TN + HRCT_Normal$FP, comb.fixed=FALSE, comb.random = TRUE, sm = "plogit", studlab=HRCT_Normal$ID)

png(filename = "HRCT_Sensitivity_rob.png", width = 3600, height = 2262, res = 400)
forest(HRCT_Sensitivity, xlim = c(0,1), pscale =1, digits = 3, fs.xlab=12, fs.study=12, fs.study.lables=12, fs.heading=12, squaresize = 0.5, col.square="navy", 
       col.square.lines="navy", col.diamond="navy", col.diamond.lines="navy", type.random="diamond", ff.fixed="bold.italic", 
       ff.random="bold.italic", print.Q=TRUE, print.pval.Q=TRUE, print.I2=TRUE, print.tau2=FALSE, 
       lty.fixed=0, lty.random=2, col.by="black", comb.fixed=FALSE, xlab="Sensitivity", lwd = 2, cex= 1.2, 
...
png(filename = "HRCT_Specificity_rob.png", width = 3600, height = 2262, res = 400)
forest(HRCT_Specificity, xlim = c(0,1), pscale =1, digits = 3, fs.xlab=12, fs.study=12, 
       fs.study.lables=12, fs.heading=12, squaresize = 0.5, col.square="navy", 
       col.square.lines="navy", col.diamond="navy", col.diamond.lines="navy", 
       type.random="diamond", ff.fixed="bold.italic", ff.random="bold.italic", 
...
# Load the saved plots
plot_hrsens_rob <- png::readPNG("HRCT_Sensitivity_rob.png")
plot_hrspec_rob <- png::readPNG("HRCT_Specificity_rob.png")

# Arrange the plots side by side
hrct_plot <- grid.arrange(
  rasterGrob(plot_hrsens_rob),
  rasterGrob(plot_hrspec_rob),
  nrow = 1,
  ncol = 2
)

# Save the combined plot
png(filename = "HRCT_Sensitivity_and_Specificity_rob.png", width = 7200, height = 2262, res = 400)
grid.draw(hrct_plot)
dev.off()
